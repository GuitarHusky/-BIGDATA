...
<div class="login-logo">
  <img src="../assets/jianongLine.png" height="44" width="71" alt="" class="logo-img">
  <span class="logo-text">嘉农云<span style="display:inline-block;margin:0px 5px;">·</span>大数据平台</span>
</div>
<div class="login-main">
  <div class="leftImg">
      <img src="../assets/LogoLeft.png" height="422" width="392" alt="">
    </div>
  <Form ref="loginForm" :model="loginForm" :rules="ruleInline" autocomplete="off" class="login-form">
    <div class="login-main-top">
    <img src="../assets/jianongLogo .png" height="44" width="152" alt="">
    </div>
    <FormItem prop="user">
    ...
    <FormItem prop="code">
        ...
        <Button class="code-btn" id='code' @click="createCode">{{code}}</Button>
    </FormItem>
    <FormItem>
        <Button type="primary" @click="handleSubmit('loginForm')" long>登录</Button>
    </FormItem>
    <FormItem>
        <span class="remember"><Checkbox label="Eat" v-model='repass'></Checkbox>下次自动登录</span><span class="reset" @click="handleReset('loginForm')">重置</span>
    </FormItem>
  </Form>
  </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.ti = sessionStorage.getItem('login')
      if(this.ti != null) {
      ...
      this.user_name = sessionStorage.getItem('usernames');
      this.user_number = sessionStorage.getItem('usernumber');
      this.user_real_name = sessionStorage.getItem('userrealname')
      this.user_email = sessionStorage.getItem('useremail')
      this.user_iphone = sessionStorage.getItem('useriphone')
    },
    methods: {
      updateInfo() {
        this.user_info = true;
      ...
      handleSubmit(name) {
        ...
        this.$refs[name].validate((valid) => {
          if(valid) {
            if(typeof this.loginForm.code == 'undefined') {
              return;
            } else {
              var inputCode = this.loginForm.code.toUpperCase();
              if(inputCode.length <= 0) {} else if(inputCode != this.code) {
                ...
                loginTo({
                  ...
                  if(res.retcode == 20000) {
                    ...
                    sessionStorage.setItem('login', 'logined')
                    sessionStorage.setItem('usernames', res.data.user.USERNAME)
                    sessionStorage.setItem('usernumber', res.data.user.NUMBER)
                    sessionStorage.setItem('userrealname', res.data.user.NAME)
                    sessionStorage.setItem('useremail', res.data.user.EMAIL)
                    sessionStorage.setItem('useriphone', res.data.user.PHONE)
                    this.user_real_name = sessionStorage.getItem('userrealname')
...
<style scoped>
 ...
 /*  .login {
   background:linear-gradient(bottom, #359A3A, #29B442) !important;
   background: #359A3A !important;
   height: 100px;
   width: 100%;
   position: absolute;
   z-index: 100;
...
.login-main .login-main-top {
  ...
  .table-bor {
    border: 1px solid rgba(206, 230, 207, 1);
  }

/* 背景 */
.login {
        height: 100vh;
        background: radial-gradient(ellipse at bottom, #218EDC 0%, #051042 100%);
        overflow: hidden;
      }
