<template>
  <div class="total-info pad">
    <Row>
      <Col span="24" class="table-title tableStyle">云检测今日提交详细
...
<script>
  import { getCloudCheckTodayCommit } from '../../../api/api'
  export default {
    data() {
      return {
        // 初始化信息总条数
    ...
    methods: {
     ...
     getRegisterList() {
        this.dataCount = 0;
        this.historyData = []
        getCloudCheckTodayCommit().then(res => {
          this.dataCount = res.result.data.length;
          this.historyData = res.result.data
          var num = 1;
          this.historyData.forEach((item, index) => {
