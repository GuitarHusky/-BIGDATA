...
<script>
  import { getReadArticleCountpvDetail } from '../../../api/api'
  export default {
    data(){
      return {
        // 初始化信息总条数
    ...
    methods: {
      ...
      getReadArticleCountpvDetailinfo() {
        getReadArticleCountpvDetail().then(res => {
          this.historyData = res.result.data;
          this.dataCount = res.result.count;
          var i = 1;
          this.historyData.forEach((item, index) => {
    ...
    mounted() {
      this.getReadArticleCountpvDetailinfo();
    }
  }
</script>
<style scoped>
