<template>
	...
	<!-- 	<div class="crumbs_me">
			<Breadcrumb separator=">">
				...
				<BreadcrumbItem>完成免疫总数 </BreadcrumbItem>
			</Breadcrumb>
		</div> -->
		<Row>
			<Col span="24" class="table-title tableStyle"> 完成免疫总数
...
<script>
	import { getCommunityInfo, getImmuneInfo } from '../../../api/api'
	export default {
		data() {
			return {
				// 初始化信息总条数
      ...
      goBack(){
        ...
        this.endtime = end.getFullYear() + '-' + (end.getMonth() + 1) + '-' + end.getDate()
				...
				getImmuneInfo({
					...
					this.historyData = res.data.immuneInfo.list
					var num = 1
