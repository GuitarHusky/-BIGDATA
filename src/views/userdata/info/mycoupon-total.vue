	...
	<div class="total-info pad">
		<!-- <div class="crumbs_me">
			<Breadcrumb separator=">">
				...
				<BreadcrumbItem>我的优惠券 </BreadcrumbItem>
			</Breadcrumb>
		</div> -->
		<Row>
			<Col span="24" class="table-title tableStyle"> 我的优惠券
      ...
      goBack(){
        ...
        this.endtime = end.getFullYear() + '-' + (end.getMonth() + 1) + '-' + end.getDate()
				...
				myCoupon({
					...
					this.$Spin.hide();
					this.dataCount = res.data.couponList.num;
					this.historyData = res.data.couponList.list
					var num = 1
					this.historyData.forEach((item, index) => {
						item['numid'] = num++
							if(item.is_activation == 1) {
								item.is_activation = '未激活'
							} else {
								item.is_activation = '已激活'
							}
						if(item.status == 0) {
							item.status = '未使用'
						}
						...
						if(item.activation_time == "0000-00-00 00:00:00") {
							item.activation_time = ''
						}
						if(item.use_time == "0000-00-00 00:00:00") {
							item.use_time = ''
						}
					})
				})
