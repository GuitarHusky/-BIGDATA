	...
	<div class="total-info pad">
		<!-- <div class="crumbs_me">
			<Breadcrumb separator=">">
				...
				<BreadcrumbItem >登录总天数</BreadcrumbItem>
			</Breadcrumb>
		</div> -->
		<Row>
			<Col span="24" class="table-title tableStyle"> 登录总天数
...
<script>
	import { getUserLoginInfo } from '../../../api/api'
	export default {
		data() {
			return {
				// 初始化信息总条数
      ...
      goBack(){
        ...
        this.endtime = end.getFullYear() + '-' + (end.getMonth() + 1) + '-' + end.getDate()
				...
				getUserLoginInfo({
					...
					this.$Spin.hide();
					this.dataCount = res.data.login_list.count;
					this.historyData = res.data.login_list.list
					var num = 1
