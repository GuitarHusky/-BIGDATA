<template>
	<div class="total-info pad">
      ...
      <Breadcrumb separator=">">
        ...
        <BreadcrumbItem>我的报价</BreadcrumbItem>
      </Breadcrumb>
    </div> -->
		<Row>
			<Col span="24" class="table-title tableStyle"> 我的报价
      ...
      <DatePicker type="daterange" :options="options2" placement="bottom-start" v-model="timeInput" confirm format="yyyy-MM-dd"  style="width: 200px;font-size:14px;margin-left:30px;margin-right:10px;"></DatePicker>
			<RadioGroup v-model="disabledGroup">
				<Radio label="1">鸡蛋报价</Radio>
				<Radio label="2">原料报价</Radio>
			</RadioGroup>
...
<script>
	import { getBatchInfo, myPrice } from '../../../api/api'
	export default {
		data() {
			return {
				// 初始化信息总条数
      ...
      goBack(){
        ...
        this.endtime = end.getFullYear() + '-' + (end.getMonth() + 1) + '-' + end.getDate()
				...
				myPrice({
					...
					this.$Spin.hide();
					this.dataCount = res.data.myPrice.num;
					this.historyData = res.data.myPrice.list
					var num = 1
        ...
        this.endtime = end.getFullYear() + '-' + (end.getMonth() + 1) + '-' + end.getDate()
				...
				if(this.disabledGroup == 2) {
					this.historyColumns[5].title = '原料报价'
				} else {
					this.historyColumns[5].title = '鸡蛋报价'
				}
				myPrice({
					tel: this.$route.query.num,
					star_time: this.begintime,
