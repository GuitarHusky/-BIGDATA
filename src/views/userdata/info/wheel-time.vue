	...
	<div class="total-info pad">
		<!-- <div class="crumbs_me">
			<Breadcrumb separator=">">
				...
				<BreadcrumbItem>大转盘使用次数</BreadcrumbItem>
			</Breadcrumb>
		</div> -->
		<Row>
			<Col span="24" class="table-title tableStyle"> 大转盘使用次数
...
<script>
	import {getWheelInfoDay,getWheelInfo } from '../../../api/api'
	export default {
		data() {
			return {
				// 初始化信息总条数
        ...
        this.endtime = end.getFullYear() + '-' + (end.getMonth() + 1) + '-' + end.getDate()
				...
				getWheelInfo({
					...
					c_page:index
					 ...
					 this.$Spin.hide();
					this.dataCount = res.data.wheelNumInfo.num;
					this.historyData = res.data.wheelNumInfo.list
					var num =1
